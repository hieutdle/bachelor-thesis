  ...
  create or replace view `bachelor-thesis-344103`.`optimized_data_warehouse`.`ER_VISITS`
  OPTIONS()
  as 
...
FROM (
	...
	FROM (
		...
		FROM `bachelor-thesis-344103`.`raw_data_warehouse`.`encounters` CL1
		JOIN `bachelor-thesis-344103`.`raw_data_warehouse`.`encounters` CL2

  ...
  create or replace table `bachelor-thesis-344103`.`optimized_data_warehouse`.`condition_era`
...
WITH ConditionTarget AS(
    ...
    FROM `bachelor-thesis-344103`.`optimized_data_warehouse`.`condition_occurrence` co
),
EndDates AS (
    SELECT
        person_id, 

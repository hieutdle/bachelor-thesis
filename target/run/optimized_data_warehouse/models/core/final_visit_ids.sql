  ...
  create or replace table `bachelor-thesis-344103`.`optimized_data_warehouse`.`final_visit_ids`
	...
	FROM (
		SELECT *,
			CASE
				...
				ELSE 99
			END AS PRIORITY
	FROM `bachelor-thesis-344103`.`optimized_data_warehouse`.`assign_all_visit_ids`
	) T1
) T2
WHERE RN=1
  );

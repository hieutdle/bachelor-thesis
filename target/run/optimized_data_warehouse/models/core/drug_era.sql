  ...
  create or replace table `bachelor-thesis-344103`.`optimized_data_warehouse`.`drug_era`
...
WITH PreDrugTarget AS(
    ...
    FROM `bachelor-thesis-344103`.`optimized_data_warehouse`.`drug_exposure` d
        INNER JOIN `bachelor-thesis-344103`.`optimized_data_warehouse`.`concept_ancestor` ca ON CAST(ca.descendant_concept_id AS STRING) = CAST(d.drug_concept_id AS STRING)
        INNER JOIN `bachelor-thesis-344103`.`optimized_data_warehouse`.`concept` c ON CAST(ca.ancestor_concept_id AS STRING)= CAST(c.concept_id AS STRING)

  ...
  create or replace table `bachelor-thesis-344103`.`optimized_data_warehouse`.`measurement`
...
with tmp as (
    select
        p.person_id,
        coalesce(srctostdvm.target_concept_id,CAST(0 AS STRING)) AS measurement_concept_id,
        ...
        CAST(0 AS STRING) as value_as_concept_id,
        CAST(0 AS STRING) as unit_concept_id,
        null as range_low,
        null as range_high,
        0 as provider_id,
        fv.visit_occurrence_id_new AS visit_occurrence_id,
        0 as visit_detail_id,
        CAST(pr.code AS STRING) AS measurement_source_value,
        coalesce(srctosrcvm.source_concept_id,CAST(0 AS STRING)) AS measurement_source_concept_id,
        ...
        select
        p.person_id,
        coalesce(srctostdvm.target_concept_id,CAST(0 AS STRING)) AS target_concept_id,
        ...
        coalesce(srcmap2.target_concept_id,CAST(0 AS STRING)) AS value_as_concept_id,
        coalesce(srcmap1.target_concept_id,CAST(0 AS STRING)) AS unit_concept_id,
        null ,
        null ,
        0 as provider_id,
        fv.visit_occurrence_id_new AS visit_occurrence_id,
        ...
        from `bachelor-thesis-344103`.`raw_data_warehouse`.`observations` o
        inner join `bachelor-thesis-344103`.`optimized_data_warehouse`.`source_to_standard_vocab_map`  srctostdvm
        on srctostdvm.source_code             = cast(o.code AS STRING)
        and srctostdvm.target_domain_id        = 'Measurement'
        and srctostdvm.source_vocabulary_id    = 'LOINC'
        and srctostdvm.target_standard_concept = 'S'
        and (srctostdvm.target_invalid_reason IS NULL OR srctostdvm.target_invalid_reason = '')
        left join `bachelor-thesis-344103`.`optimized_data_warehouse`.`source_to_standard_vocab_map`  srcmap1
        on srcmap1.source_code                = cast(o.units AS STRING)
        ...
        left join `bachelor-thesis-344103`.`optimized_data_warehouse`.`source_to_standard_vocab_map`  srcmap2
        on srcmap2.source_code                = cast(o.value AS STRING)
        ...
        left join `bachelor-thesis-344103`.`optimized_data_warehouse`.`source_to_source_vocab_map` srctosrcvm
        on srctosrcvm.source_code             = Cast(o.code AS STRING)
